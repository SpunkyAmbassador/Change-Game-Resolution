/**
 * This plugin allows to set the window resolution and full screen mode.
 */

// Plugin name
const pluginName = 'Change Game Resolution';

const systemJSONFilePath = Common.Paths.FILE_SYSTEM;

/**
 * Set the window size to the specified width, height and full screen mode.
 * @param {number} width - The width of the window.
 * @param {number} height - The height of the window.
 * @param {boolean} fullscreen - Indicates if the window is in full screen mode.
 */
async function setWindowSize(width, height, fullscreen) {
    const data = await Common.IO.parseFileJSON(systemJSONFilePath);

    // Modify values
    data.sh = height;
    data.sw = width;
    data.isw = !fullscreen;

    // Save object directly
    await Common.Platform.writeFile(systemJSONFilePath, data);

    // Update the window size
    Data.Systems.updateWindowSize(width, height, fullscreen);
    Data.Systems.updateWindowSize(width, height, fullscreen);
}

/**
 * Set the desired screen resolution.
 * @param {number} Width - The width of the screen.
 * @param {number} Height - The height of the screen.
 */
Manager.Plugins.registerCommand(pluginName, 'Set Resolution', (Width, Height) => {
    setWindowSize(Width, Height, false);
});

/**
 * Set the game to full screen mode.
 */
Manager.Plugins.registerCommand(pluginName, 'Set Full Screen', () => {
    setWindowSize(1280, 720, true);
});

/**
 * Confirm the resolution change.
 */
Manager.Plugins.registerCommand(pluginName, 'Reload Game', () => {
    // Reload the window/game
    window.location.reload();
});
